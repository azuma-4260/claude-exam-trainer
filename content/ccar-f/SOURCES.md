...
| 2026-09-19 | 選択肢バランス是正(オーナー指摘): active な MCQ 208 問中 206 問で正解が最長、正解の 83% にカンマ(誤答 21%)など、形で正解が推測できる偏りを確認。全 active MCQ の誤答を意味を変えずに正解と同等の長さ・構造・記法へ書き直し(editorial fix、rev++、status は active 維持)。`scripts/audit-choice-balance.ts`(`npm run audit:choices`)を追加し CI で fail closed に検査(specs/06 / 07 更新) |
